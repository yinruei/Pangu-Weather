...
for time, upper_data in data_per_time_upper:
    ...
    # 计算850 hPa的风场数据
    u_wind_850 = upper_data[upper_variables.index('u_component_of_wind'), pressure_levels.index('850'), :, :]
    v_wind_850 = upper_data[upper_variables.index('v_component_of_wind'), pressure_levels.index('850'), :, :]
    ...
    # 绘制风场数据
    # q = ax.quiver(lon, lat, u_wind_850, v_wind_850, transform=ccrs.PlateCarree(), scale=500, color='black')

    # 绘制850 hPa风场的shading图
    cf = ax.contourf(lon_grid, lat_grid, wind_speed_850, transform=ccrs.PlateCarree(), cmap='viridis', levels=20)
    ...
    q = ax.quiver(lon, lat, u_wind_850, v_wind_850, transform=ccrs.PlateCarree(), regrid_shape=35, width=0.002)
    ...
    # 调整 title 位置
    fig.suptitle(f"850 hPa Wind + Sea Level Pressure at Time: {time}", fontsize=font_size, y=pos.y1 + 0.05)

    # 构建文件名，使用时间
    filename = f"{output_folder}/850wind_slp_{time.strftime('%Y%m%d_%H%M')}_asia.png"
