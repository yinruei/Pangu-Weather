...
for time, upper_data in data_per_time_upper:
    ...
    # 计算200 hPa的风场数据
    u_wind_200 = upper_data[upper_variables.index('u_component_of_wind'), pressure_levels.index('200'), :, :]
    v_wind_200 = upper_data[upper_variables.index('v_component_of_wind'), pressure_levels.index('200'), :, :]
    wind_speed_200 = np.sqrt(u_wind_200**2 + v_wind_200**2)
    ...
    # 绘制风场数据
    # q = ax.quiver(lon, lat, u_wind_200, v_wind_200, transform=ccrs.PlateCarree(), scale=500, color='black')

    # 绘制200 hPa风场的shading图
    cf = ax.contourf(lon_grid, lat_grid, wind_speed_200, transform=ccrs.PlateCarree(), cmap='viridis', levels=20)
    
    
    # 添加风场的 extended color bar
    cbar_ax = fig.add_axes([ax.get_position().x1 + 0.01, ax.get_position().y0, 0.02, ax.get_position().height])
    cbar = plt.colorbar(cf, cax=cbar_ax, orientation='vertical', extend='both')
    cbar.set_label('Wind Speed (m/s)', fontsize=font_size)
    
    q = ax.quiver(lon, lat, u_wind_200, v_wind_200, transform=ccrs.PlateCarree(), regrid_shape=35, width=0.002)
    ...
    # 调整 title 位置
    fig.suptitle(f"200 hPa Wind + Sea Level Pressure at Time: {time}", fontsize=font_size, y=pos.y1 + 0.05)

    # 构建文件名，使用时间
    filename = f"{output_folder}/200wind_slp_{time.strftime('%Y%m%d_%H%M')}_asia.png"
